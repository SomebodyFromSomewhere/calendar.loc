$(document).ready(function () {
    ...
    let day = parseInt($(".curr_day :selected").val());
    let prevBtn = $("#prevDay");
    let nextBtn = $("#nextDay");
    let dayBtn = $(".curr_day");
    ...
    dayBtn.click(
        dayBtn.change(function () {
            let year = parseInt($(".year option:selected").val());
            let month = parseInt($(".curr_month option:selected").val());
            let day = parseInt($(".curr_day option:selected").val());
            window.location.replace("http://calendar.loc/schedule.php?day=" + day + "&month=" + month + "&year=" + (year));
        })
    );
    monthBtn.click(
